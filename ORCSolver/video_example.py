...
import time
from legacy_cli import configure
...
window_width = 400
window_height = 640
cli = configure(window_width, window_height, show_window)
window_width, window_height, show_window = cli.width, cli.height, cli.show_window
...
def main():
    ...
    cli.log(window_width, window_height)
    ...
    cli.report(elapsed, best_leaf_result, best_leaf_loss)
    ...
    for i in range(len(verticallogo_result_index)):
        if isinstance(verticallogo_row_width[i], list):
            ...
            for j in range(len(verticallogo_result_index[i])):               
                ...
                if show_window:
                    cli.log(verticallogo_result_index[i][j])
                    widgets[verticallogo_result_index[i][j]+num_top+num_left][0].place(x=left, y=top, width=widget_width, height=widget_height)
                top += widget_height
            left += widget_width
            top = VL_t
